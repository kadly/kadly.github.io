...
const telescopiccrawlercranes = [
  "ZCT300V", "ZCT600V", "ZCT900V", "ZCT1500V"
];

const TelescopiccrawlercranesPage = () => {
  ...
  const images = useStaticQuery(graphql`
    query {
      allFile(filter: { relativeDirectory: { eq: "telescopic-crawler-cranes" } }) {
        edges {
          node {
            name
            childImageSharp {
  ...
  return (
    <Layout>
      <Header>
        <h1>Краны гусеничные с телескопической стрелой</h1>
        <Link to="/">
          <h2>Zoomlion</h2>
        </Link>
        <ContactInfo>
      ...
      <Container>
        <Grid>
          {telescopiccrawlercranes.map((model) => (
...
export default TelescopiccrawlercranesPage;
