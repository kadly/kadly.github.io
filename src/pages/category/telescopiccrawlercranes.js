...
const TelescopiccrawlercranesPage = ({ data }) => {
  ...
  return (
    <Layout>
      <Header centerText="Телескопические гусеничные краны" />
...
export const query = graphql`
  query {
    images: allFile(filter: { relativeDirectory: { eq: "telescopiccrawlercranes" } }) {
      edges {
        node {
          name
          childImageSharp {
    ...
    models: allTelescopiccrawlercranesJson {
      edges {
        node {
          modelId
          name
